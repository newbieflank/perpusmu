...
public class Buku extends javax.swing.JPanel {
    ...
    public Buku() throws SQLException {
        ...
        txt_search.putClientProperty(FlatClientProperties.PLACEHOLDER_TEXT, "Cari Buku");
...
//        cek jika tidak ada row yang diselect
    ...
    private void dial_tahunKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_dial_tahunKeyTyped
        // TODO add your handling code here:
        String angkat = dial_tahun.getText();
        int angka = angkat.length();
        if (!(Character.isAlphabetic(evt.getKeyChar()) || (Character.isWhitespace(evt.getKeyChar())))) {
            if (angka >= 4) {
                JOptionPane.showMessageDialog(Jdialog, "Hanya Bisa di Isi 4 Karakter");
                evt.consume();
            }
        } else if (Character.isWhitespace(evt.getKeyChar())) {
            JOptionPane.showMessageDialog(Jdialog, "Tidak Boleh ada Spasi");
            evt.consume();
        } else {
            JOptionPane.showMessageDialog(Jdialog, "Hanya Bisa di isi dengan Angka");
            evt.consume();
        }
